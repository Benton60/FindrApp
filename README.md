...
The project was never meant to become a fully fledged product; it was meant to provide a means to experiment with new technologies
...
FindrApp is the git repository for the frontend. It is written as an Android client. You'll quickly notice that while most files are in Kotlin, some files are written in Java.
...
Rather than switching entire activities when switching between the map and home screens. I used fragments. Using fragments allowed me to keep a consistent button ribbon
...
Camera Fragment - allows the user to take pictures and make posts, as well as open the Photo Preview Fragment that sends posts to the API
Map Fragment - this currently only displays the locations of the user's friends. It connects to Google's Map API.
